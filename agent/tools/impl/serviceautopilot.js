...
// Client-record fields SaveClient will never be called with via saveClientFields —
// billing/tax/payment-method-adjacent fields, per the standing rule that SA is the
// source of truth for billing and only metadata fields are ever safe to auto-edit.
// This is a defense-in-depth guard on top of callers only ever passing safe fields.
const CLIENT_BILLING_SENSITIVE_FIELDS = new Set([
  'SalesTaxCodeID', 'SalesTaxRefID', 'PaymentMethodID', 'AutoCharge', 'BillingDate',
  'AccountNumber', 'CCNumber', 'CCExpiration', 'CCToken', 'CCCustomerToken', 'CCBrand',
  'SubscriptionType', 'InvoiceFrequencyID', 'StandardTermID',
]);

/**
 * General-purpose SA client-record read-modify-write, analogous to `saveInvoiceFields`.
 * Round-trips the full record from GetClientInfo unchanged except for whatever's in
 * `overrides` — use this instead of writing one-off full-payload SaveClient calls by
 * hand (see setClientBillingDefaults/setClientCrackfill, which predate this and build
 * the same payload shape inline). `expect` is an optional {field: expectedValue} map
 * checked against a fresh GetClientInfo read after saving; throws if any field doesn't
 * match. Refuses to touch billing/tax/payment-method fields (see
 * CLIENT_BILLING_SENSITIVE_FIELDS) — callers needing those must edit `SaveClient`
 * payloads directly with explicit sign-off, not through this generic helper.
 */
export async function saveClientFields({ clientId, overrides = {}, expect = {} }) {
  const badFields = Object.keys(overrides).filter(f => CLIENT_BILLING_SENSITIVE_FIELDS.has(f));
  if (badFields.length > 0) {
    throw new Error(`SA saveClientFields: refusing to write billing-sensitive field(s) ${badFields.join(', ')} — SA is the source of truth for billing`);
  }
  ...
  // Preserve every existing value as-is (unlike setClientBillingDefaults, this does NOT
  // force any tax/invoice-delivery defaults) — overrides are applied last.
  const info = {
    ClientID:                clientId,
    IsLead:                  false,
    saveType:                0,
    ...
    CountryID:               d.CountryInfo?.Value               || EMPTY_GUID,
    DefaultBillingUnderID:   d.BillingUnderInfo?.Value          || EMPTY_GUID,
    ClientSinceDate:         parseSaDateForSave(d.ClientSinceDate),
    CSRId:                   d.CSRInfo?.Value                   || EMPTY_GUID,
    AccountTypeID:           d.AccountTypeInfo?.Value           || EMPTY_GUID,
    PriorityID:              d.PriorityID                       || 0,
    UserName:                d.UserName                         || '',
    ...
    AccountNumber:           d.AccountNumber                    || '',
    SubscriptionType:        d.SubscriptionType                 || 0,
    BillingDate:             parseSaDateForSave(d.BillingDate),
    AutoCharge:              d.AutoCharge                       || false,
    BillingNotes:            d.BillingNotes                     || '',
    PaymentMethodID:         d.PaymentMethodInfo?.Value         || EMPTY_GUID,
    SalesTaxCodeID:          d.SalesTaxCodeInfo?.Value          || EMPTY_GUID,
  ...
  const errors = result?.response?.Errors;
  if (errors?.length > 0) {
    throw new Error(`SA saveClientFields errors: ${JSON.stringify(errors)}`);
  }

  // SA returns 500 for QBO-synced clients even on a successful save (known server-side
  // bug in post-save QBO sync code) — always verify via a fresh read rather than
  // trusting the HTTP status.
  if (saveRes.status === 500) {
    logger.warn('SA: saveClientFields SaveClient returned 500 (known SA QBO-sync bug) — verifying via GetClientInfo', { clientId });
  }

  const verifyRes = await post('/webservices/ClientEditOverlayWs.asmx/GetClientInfo',
    { ClientID: clientId }, 'ClientView.aspx');
  const verify = verifyRes.data?.d;
  if (!verify) throw new Error(`SA saveClientFields: verification GetClientInfo failed for ${clientId}`);
  for (const [field, expected] of Object.entries(expect)) {
    if (verify[field] !== expected) {
  ...
  logger.info('SA: saveClientFields complete', { clientId, fields: Object.keys(overrides), savedViaApi: saveRes.status !== 500 });
  return verify;
}
