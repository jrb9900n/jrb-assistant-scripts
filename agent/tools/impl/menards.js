...
async function fillAndPrintRebateForm(expense) {
  ...
  try {
    ...
    // Email PDF to Michael
    const pdfBuffer = fs.readFileSync(pdfPath);
    const fileName  = path.basename(pdfPath);
    await sendEmail({
      to: ['michael@jrboehlke.com'],
      subject: `Menards Rebate Form — ${expense.vendor || 'Menards'} $${Number(expense.amount || 0).toFixed(2)}`,
      body: `<p>Menards rebate form for the <strong>$${Number(expense.amount || 0).toFixed(2)}</strong> charge at ${expense.vendor || 'Menards'} on ${expense.transaction_date || 'recent date'}.</p><p>Print, attach the original receipt, and mail to the address on the form.</p><p><em>— JRB Assistant</em></p>`,
