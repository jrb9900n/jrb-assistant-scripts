...
4. After ALL jobs are dispatched, call `sa_update_route_order` once per day+crew with `job_ids` in stop order (same order as the draft). This sets the stop sequence numbers on the SA dispatch board.
