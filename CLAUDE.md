...
## SA Client Categorization (built 2026-08-19/20)
...
Tuned twice against real data: once against the first live dry-run's low-confidence bucket, and again against ~269 rows Michael manually corrected in a review spreadsheet. Key lessons from that second pass (2026-08-19):
...
**`node_modules` in this shared directory emptied itself twice** during the run (once mid-backfill, once between chunks) — most likely concurrent-session contention (another session or the `branch_drift_check` cron's `git pull` racing an `npm` operation), not a code bug. The live Teams bot process never restarted during either incident (Node keeps loaded modules in memory), so there was no live-service impact, but any fresh `node` invocation during the empty window fails hard. Both times fixed with a plain `npm install`. Worth a watchful eye if it recurs.
