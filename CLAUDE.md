...
### Required manual step (done 2026-08-21)
`launcher/start-agent.ps1` now injects `QB_REFRESH_TOKEN_TRANSPORT`/`QB_REALM_ID_TRANSPORT` (Michael approved the launcher edit; landed via PR #304, `claude/qbo-transport-launcher-secrets`). Michael completed the one-time OAuth for JRB Transport the same day — `QB_REFRESH_TOKEN_TRANSPORT` is in Credential Manager, and the realm ID (`9130357740125736`) is persisted in `data/qb-token-meta-transport.json` (`getQBRealmId()` falls back to this meta file when the Credential Manager entry for `QB_REALM_ID_TRANSPORT` isn't set — confirmed live, this is expected, not a gap). Both companies' QBO connections are fully live.

### Scope note (updated 2026-08-21)
`tools/impl/ap-report.js` (Wednesday 9:30 AM Accounts Payable report) is now multi-entity — fetches `getAPAgingReport()` for both `'jrb'` and `'transport'`, tags every bill with its entity, and independently try/catches each company's fetch so one QBO hiccup doesn't blank the whole report. Investigated live whether intercompany elimination was needed at the bill level: confirmed it isn't — the real intercompany relationship ("Vehicle Transportation & Management" expense / "Management & Employment Expenses") is tracked entirely through dedicated "Due to/from" balance-sheet accounts via journal entries on both companies' books, never through the vendor/Bill system (zero open bills against vendor "JRB Transport LLC" in JRB's books; J.R. Boehlke isn't even set up as a vendor in Transport's books). `EXCLUDED_INTERCOMPANY_VENDORS` in that file is a defensive no-op guard for if that ever changes. Note: a real ~$700 mismatch was found between the two companies' "Due To/From ... Transportation" account balances (not a rounding artifact) — flagged to Michael directly, not built into any report; worth a bookkeeper reconciliation pass.

All other report generators (AR aging, cash forecast, commission engine, etc.) still call `quickbooks.js` exports with the implicit `'jrb'` default and have not been made multi-entity. CardDAV (`tools/impl/carddav.js`) is also unchanged — JRB-only by design.
