...
import { supabase, gatherSAARaging, gatherSAFreshness, mondayOf, f$, fD, sectionHeader, alertBox } from './ar-report-helpers.js';
...
function buildEmail({ weekStart, weekWindows, startingCash, cashAccounts, arForecast, apForecast, apBills, payrollEstimate, rows, lowest, freshness, priorComparison }) {
...
<tr><td style="padding:28px 32px;">`;
  ...
  html += alertBox('#f0f4ff', '#1a1a2e', 'This Is An Estimate, Not A Guarantee', `
    ...
    <ul style="margin:0;padding-left:18px;font-size:12px;color:#333333;line-height:1.6;">
      <li><b>Starting cash</b> = sum of QuickBooks bank account balances (${cashAccounts.map(a => `${a.name}: ${f$(a.balance)}`).join(', ')}) as of the most recent QuickBooks read (may be up to ~1hr cached, not the current second). Not a full balance-sheet reconciliation (outstanding checks, undeposited funds, etc. are not adjusted for).</li>
      <li><b>AR collections</b> forecast ONLY today's already-outstanding AR — it does NOT project new sales/invoicing that will happen during these 12 weeks. Heuristic: ${Math.round(CURRENT_D30_NEAR_TERM_PCT * 100)}% of the Current + 1-30d balance is assumed to collect within the first ${CURRENT_D30_NEAR_TERM_WEEKS} weeks, the remainder over the following ${CURRENT_D30_REMAINDER_WEEKS} weeks; only ${Math.round(D60_COLLECTION_PCT * 100)}% of the 31-60d bucket and ${Math.round(D90PLUS_COLLECTION_PCT * 100)}% of 61d+ is assumed to ever convert to cash in this window (those accounts are the AR/Collections report's call queue, not reliable forecast cash).</li>
      <li><b>AP due</b> = bills currently entered as open Bills in QuickBooks, bucketed by due date. Anything not yet entered as a Bill (e.g. a cash purchase, a subscription charged directly to a card) is not captured here.${apForecast.beyondWindowCount ? ` ${apForecast.beyondWindowCount} open bill(s) totaling ${f$(apForecast.beyondWindowTotal)} are due beyond this 12-week window and are excluded.` : ''}</li>
      <li><b>Payroll</b> = ${f$(payrollEstimate.weeklyAverage)}/week, the trailing ${payrollEstimate.lookbackWeeks}-week average of actual "Payroll Payable" cash postings in QuickBooks — a smoothed average, NOT a real ADP pay calendar (this integration doesn't have QuickBooks Payroll API access, only the regular Accounting API). Applied flat to every week. Employer payroll taxes/ADP fees/benefits are excluded (posted to separate accounts; captured via the AP line only if they're billed as a vendor Bill).</li>
      <li>A fuller AP Aging report is being built separately (PR #295 / <code>claude/ap-report</code>, not yet merged) — this report uses its own minimal open-bills query so it doesn't depend on unmerged code; worth revisiting once that lands.</li>
    </ul>`);

  // ── Weekly forecast table ────────────────────────────────────────────────
  html += sectionHeader('12-Week Projection');
  html += `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:8px;font-size:12px;">
  ...
  // ── AP detail ─────────────────────────────────────────────────────────────
  html += sectionHeader(`Open Bills Feeding This Forecast (${apBills.length})`);
  ...
  html += `<p style="margin:0 0 16px;font-size:11px;color:#888888;">Source: QuickBooks open Bills (Balance &gt; 0), bucketed by due date. Full vendor-level AP aging is a separate report in progress (see assumptions above).</p>`;
  ...
  if (priorComparison) {
    ...
    html += sectionHeader(`Last Week's Forecast vs. Reality`);
    html += `<p style="margin:0 0 16px;font-size:12px;color:#333333;">Last week's forecast (${fD(priorComparison.weekStart)}) projected this week's starting cash at ${f$(priorComparison.predictedEnding)}. Actual: ${f$(priorComparison.actualStarting)}. <span style="color:${color};font-weight:bold;">Variance: ${variance >= 0 ? '+' : ''}${f$(variance)}</span>.</p>`;
  } else {
...
export async function generateAndSendCashForecastReport() {
  ...
  const [arAging, freshness, cashBalance, apBills, payrollEstimate] = await Promise.all([
    gatherSAARaging(),
    gatherSAFreshness({ staleHours: STALE_DATA_HOURS }),
    getCashBalance(),
    getOpenBillsForForecast(),
    getPayrollCashOutflowEstimate({ lookbackDays: PAYROLL_LOOKBACK_DAYS }),
  ]);

  const arForecast = buildARCollectionForecast(arAging);
  const apForecast = buildAPForecast(apBills, weekWindows);

  const rows = [];
  let running = cashBalance.total;
  for (let i = 0; i < FORECAST_WEEKS; i++) {
    const arIn = arForecast.weekly[i];
  ...
  const lowestWithIndex = { ...lowest, index: lowestIndex };

  const priorComparison = await gatherAndRecordSnapshot({
    weekStart,
    startingCash: cashBalance.total,
    weeklyRows: rows,
    lowestWeek: lowest.weekStart,
    lowestAmount: lowest.ending,
  });

  const body = buildEmail({
    weekStart, weekWindows, startingCash: cashBalance.total, cashAccounts: cashBalance.accounts,
    arForecast, apForecast, apBills, payrollEstimate, rows, lowest: lowestWithIndex, freshness, priorComparison,
  });

  await sendEmail({
    to: ['michael@jrboehlke.com'],
    subject: `12-Week Cash Forecast — Week of ${fD(weekStart)} | Low point ${f$(lowest.ending)}`,
    body,
  });

  logger.info('cash_forecast_report: sent', { weekStart, startingCash: cashBalance.total, lowestEnding: lowest.ending, lowestWeek: lowest.weekStart });
  return { weekStart, startingCash: cashBalance.total, lowestEnding: lowest.ending, lowestWeek: lowest.weekStart };
}
