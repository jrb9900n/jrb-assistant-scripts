...
const SCHEDULING_TOOLS = [
  ...
  },
  {
    ...
    description: 'Persist a scheduling rule or correction so it applies to all future sessions. Use when Michael corrects a mistake or states a standing rule (e.g. "Dave never does X", "always ask about Y"). The rule is stored in Supabase and injected into every future scheduling system prompt.',
    input_schema: {
      type: 'object',
      properties: {
        rule: { type: 'string', description: 'The rule to save, written as a clear, actionable statement.' },
      },
      required: ['rule'],
    },
  },
