# save-openai-secrets.ps1
# Run this in your own PowerShell window to store the OpenAI API key
# in Windows Credential Manager. The value is prompted interactively and
# never passes through Claude Code / chat.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File "C:\Users\Assistant\JRBAgent\launcher\save-openai-secrets.ps1"
...
Write-Host "`nOpenAI - Credential Manager Setup" -ForegroundColor Cyan
Write-Host "=====================================`n"

Write-Host "OPENAI_API_KEY - API key from platform.openai.com, used for Whisper (voice memo transcription) and TTS (voice replies) in Teams"
Set-JRBSecret "OPENAI_API_KEY"

Write-Host "`nDone. This is now readable as JRBAgent:OPENAI_API_KEY."
